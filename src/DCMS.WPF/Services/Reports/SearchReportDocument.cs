...
public class SearchReportDocument : BaseReportDocument
{
    private readonly IEnumerable<SearchItem> _items;

    public SearchReportDocument(string title, IEnumerable<SearchItem> items)
        : base(title)
    {
        _items = items;
    }
    ...
    private void ComposeContent(IContainer container)
    {
        container.PaddingVertical(15).Table(table =>
        {
            table.ColumnsDefinition(columns =>
            {
                ...
                columns.RelativeColumn(1);    // Transferred To
                columns.RelativeColumn(1.5f); // Reply
            ...
            table.Header(header =>
            {
                ...
                header.Cell().Element(CellStyle).Text("محول إلى");
                header.Cell().Element(CellStyle).Text("الرد");
            ...
            foreach (var item in _items)
            {
                ...
                table.Cell().Element(CellStyle).Text(item.TransferredTo);
                table.Cell().Element(CellStyle).Text(item.Reply ?? "-");
